...
class ExpandTitle extends React.Component {
  ...
  render() {
    const {
      children: title,
      item: { newsShortContent, newsTitle, newsId, newsSourceLink },
    } = this.props;
    const { isShowExpand } = this.state;

    const className = `content-expand ${isShowExpand ? 'show' : ''}`;

    return (
      <div
        className="text-left expand-title-wrap"
        onClick={() => this.toggleExpand(title)}
      >
        <div className="header-title">
          <a href={`#${newsId}`} style={{ cursor: 'pointer' }}>
            {newsTitle}
          </a>
        </div>
