...
class TablePressing extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      page: 1,
  ...
  getDataFromRedux = state =>
    state[REDUCER_NAME].listObjectDeceptivePressingDataByTicker;
  ...
  schema = (listColumn, i18nTitleKey) =>
    Object.keys(Const.listColumn).map((item, index) => {
      ...
      if (_.includes([listColumn.BID], key)) {
        return {
          ...result,
          render: (text, item) => {
            return (
      ...
      if (
        _.includes(
          [
            listColumn.AVG_VOLUME_10D,
            listColumn.PRESSING_VOL_BUY,
            listColumn.PRESSING_VOL_SELL,
          ],
          key,
        )
      ) {
  ...
  render() {
    ...
    return (
      <Table
        ...
        schema={this.schema(Const.listColumn, 'tasignals.pressingHeader')}
      />
    );
  }
}
...
const mapStateToProps = state => {
  return {
    ids: state[REDUCER_NAME].listDeceptivePressingDataTicker,
...
export default enhance(TablePressing);
