...
class TableClosing extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      page: 1,
  ...
  getDataFromRedux = state =>
    state[REDUCER_NAME].listObjectDeceptiveClosingDataByTicker;
  ...
  render() {
    ...
    const listId = this.getPaginationInfo();
    ...
    const isBefore2h = startTime < currentTime && currentTime < openTime;
    if (!isBefore2h) {
      if (listId.length === 0 && !isLoading) {
        return (
          <div
    ...
    return (
      <Table
        ...
        schema={this.schema(Const.listColumn, 'tasignals.closingHeader')}
        noDataText={
          isBefore2h
            ? I18n.t('tasignals.deceptiveDataMessage.nodataClosing')
            : ''
        }
      />
    );
...
const mapStateToProps = state => {
  return {
    ids: state[REDUCER_NAME].listDeceptiveClosingDataTicker,
...
const enhance = compose(
  withScroll(280, REDUCER_NAME),
  connect(mapStateToProps),
);

export default enhance(TableClosing);
