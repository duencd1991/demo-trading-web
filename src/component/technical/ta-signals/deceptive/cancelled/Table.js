...
class TableCancelled extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      page: 1,
  ...
  getDataFromRedux = state =>
    state[REDUCER_NAME].listObjectDeceptiveCancelledDataByTicker;
  ...
  render() {
    ...
    if (ids.length === 0 && !isLoading) {
      return (
        <div
          style={{ height: 'calc(100vh - 350px)', minHeight: '10px' }}
    ...
    return (
      <Table
        ...
        schema={this.schema(Const.listColumn, 'tasignals.cancelledHeader')}
      />
    );
  }
}
...
const mapStateToProps = state => {
  return {
    ids: state[REDUCER_NAME].listDeceptiveCancelledDataTicker,
...
const enhance = compose(
  withScroll(240, REDUCER_NAME),
  connect(mapStateToProps),
);

export default enhance(TableCancelled);
