...
var StackedBarSeries = function (_Component) {
  ...
  _createClass(StackedBarSeries, [
    ...
    value: function drawOnCanvas(ctx, moreProps) {
      ...
      drawOnCanvasHelper(ctx, this.props, moreProps, xAccessor,stack.default);
    }
  }, {
    key: "renderSVG",
    value: function renderSVG(moreProps) {
      ...
      return _react2.default.createElement(
        "g",
        null,
        svgHelper(this.props, moreProps, xAccessor, stack.default)
      );
    }
  }, {
    key: "render",
...
function drawOnCanvasHelper(ctx, props, moreProps, xAccessor, stackFn) {
  ...
  var bars = doStuff(props, xAccessor, plotData, xScale, yScale, stackFn, postRotateAction, defaultPostAction, hovering);
...
function doStuff(props, xAccessor, plotData, xScale, yScale, stackFn, postRotateAction, defaultPostAction, hovering) {
  ...
  var bars = getBars(props, modifiedXAccessor, modifiedYAccessor, modifiedXScale, modifiedYScale, plotData, stackFn, postProcessor, hovering);
    ...
    nest.forEach(function (outer) {
      ...
      */
          ...
          if (d.valueY && d.title) {
            ...
            if (d.valueY > 0) {
              ctx.fillText(d.title, d.x + d.width / 2 - ctx.measureText(d.title).width / 2, 210);
            } else {
              ctx.fillText(d.title, d.x + d.width / 2 - ctx.measureText(d.title).width / 2, 180);
            }
          }
          ...
          if (d.valueX && d.title) {
            ...
            const padding = 10
            if (d.title > 0) {
              ctx.fillText(d.title, d.x + d.width + offset, d.y + (d.height + (padding / 2)) / 2);
            }
            if (d.title < 0) {
              ctx.fillText(d.title, d.x - offset / 2, d.y + (d.height + (padding / 2)) / 2);
            }
          }
          // todo: end draw title
        }
...
function getBars(props, xAccessor, yAccessor, xScale, yScale, plotData, stackFn, postProcessor, hovering) {
  ...
  var ds = plotData.map(function (each) {
    ...
    yAccessor.forEach(function (eachYAccessor, i) {
      ...
      var appearance = {
        ...
        stroke: stroke ? fillFuc(each, i) : "none",
        fill: fillFuc(each, i)
      };
