...
var _index = require("react-stockcharts/lib/utils");
...
function getCurrentItem(xScale, xAccessor, mouseXY, plotData) {
    ...
    let min = xScale.range()[0];
    let max = xScale.range()[1];
    let length = plotData.length;

    let barRange = (max - min) / length;

    let distanceVsMin = mouseXY[0] - min;
    let index = Math.floor(distanceVsMin / barRange);

    // item = plotData[index];
    var d = {x: mouseXY[0], idx: index}
