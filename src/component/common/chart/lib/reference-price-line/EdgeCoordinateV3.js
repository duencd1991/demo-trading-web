...
function helper(props) {
  ...
  if ((0, _utils.isDefined)(displayCoordinate)) {
    var textAnchor = 'right'; // TODO: Below it is necessary to implement logic for the possibility of alignment from the right or from the left.
...
function drawOnCanvas(ctx, props) {
  ...
  ctx.font =
    ...
    fontSize +
    'px ' +
    fontFamily; /* ========== CUSTOM CODE: set default font to bold ========= */
  ...
  if ((0, _utils.isDefined)(edge.coordinateBase)) {
    ...
    if (edge.orient === 'right') {
      /* ========== CUSTOM CODE START========= */
      /* round border rectangular  */
      ctx.lineJoin = 'round';
      ctx.lineWidth = rectRadius;
      /* ========== CUSTOM CODE END ========== */
    ...
    /* ========== CUSTOM CODE START========= */
    /* Reference price should be bold in front of '.'  */
    ctx.textAlign = 'right';
    ctx.textBaseline = 'alphabetic';
    var price = edge.coordinate.displayCoordinate.split('.');
    var priceBefore = price[0];
    var priceAfter = price[1];
    ctx.fillText(
      priceBefore,
      edge.coordinate.edgeXText + 0.1 * rectWidth,
      edge.coordinate.edgeYText + fontSize / 3,
    );

    ctx.textAlign = 'left';
    ctx.font = (fontSize * 3) / 4 + 'px ' + fontFamily; // font size 9px
    ctx.fillText(
      '.' + priceAfter,
      edge.coordinate.edgeXText + 0.1 * rectWidth,
      edge.coordinate.edgeYText + fontSize / 3,
