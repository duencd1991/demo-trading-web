...
var HoverTooltip = (function(_Component) {
  ...
  function HoverTooltip(props) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (HoverTooltip.__proto__ || Object.getPrototypeOf(HoverTooltip)).call(
        this,
        props,
      ),
    );
  ...
  _createClass(HoverTooltip, [
    ...
    },
    {
      ...
      value: function renderSVG(moreProps) {
        ...
        var bgShape =
          (0, _utils.isDefined)(bgwidth) && (0, _utils.isDefined)(bgheight)
...
HoverTooltip.propTypes = {
  ...
  bgFixedSize: _propTypes2.default.object, // todo: when you want to fixed tooltip size
  ...
  triangleSize: _propTypes2.default.number,
};
...
function tooltipSVG(_ref3, content) {
  ...
  for (var i = 0; i < content.y.length; i++) {
    ...
    tspans.push(_react2.default.createElement('tspan', { key: i }, ': '));
    tspans.push(
      _react2.default.createElement('tspan', { key: 'V-' + i }, y.value),
    );
  }
  return _react2.default.createElement(
    'text',
    { fontFamily: fontFamily, fontSize: fontSize, fill: fontFill },
    _react2.default.createElement('tspan', { x: X, y: startY }, content.x),
    tspans,
  );
}
/* eslint-enable react/prop-types */

function backgroundShapeCanvas(props, centerX, _ref4, ctx, width, x) {
  var w = _ref4.width,
    h = _ref4.height;
  var fill = props.fill,
    stroke = props.stroke,
  ...
  /* round border rectangular with triangle  */
  ctx.lineWidth = 1; //border width
  const r = 4;

  const triangleSize = props.triangleSize;
...
function tooltipCanvas(_ref5, content, ctx, pointer, height, moreProps, _this) {
...
function _drawOnCanvas(ctx, props, context, pointer, height, moreProps, _this) {
  // todo add more param for snapshot tool tip
  ...
  const width = moreProps.width;
  content && backgroundShapeCanvas(props, centerX, bgSize, ctx, width, x);
  content &&
    tooltipCanvas(props, content, ctx, pointer, height, moreProps, _this);

  ctx.restore();
}

function calculateTooltipSize(_ref6, content, ctx) {
  //todo: fixed tooltip size
  if (_ref6.bgFixedSize) {
    return {
      width: _ref6.bgFixedSize.width,
      height: _ref6.bgFixedSize.height,
    };
  }
  // end todo
  ...
  var measureText = function measureText(str) {
    return {
      ...
      height: fontSize, // todo padding fontSize * 0.5
    };
  };

  var _content$y$map$reduce = content.y
      .map(function(_ref7) {
  ...
  return {
    width: width + 2 * X, // todo padding X left and right
    height: height + 2 + (content.y.length - 1) * fontSize * 0.3, // todo padding 2px
  };
}

function sumSizes() {
  for (
    var _len = arguments.length, sizes = Array(_len), _key = 0;
    _key < _len;
    _key++
  ) {
...
function origin(props, moreProps, bgSize, pointWidth) {
  ...
  if (
    (0, _utils.isDefined)(chartId) &&
    (0, _utils.isDefined)(yAccessor) &&
    (0, _utils.isDefined)(chartConfig) &&
    (0, _utils.isDefined)(chartConfig.findIndex)
  ) {
  ...
  // todo: tooltip center bar column
  x = getPositionBg(x, bgSize, width).x;
  // x = normalizeX(x, bgSize, pointWidth, width);
  // todo: end tooltip center bar column

  // todo: start disable follow mouse, enable follow point data
  if (props.offsetY) {
    var yValue = yAccessor(currentItem);
    y = chartConfig.yScale(yValue) + props.offsetY;
  } else {
    y = normalizeY(y, bgSize);
  }
  // todo: end disable follow mouse, enable follow point data
...
function helper(props, moreProps, ctx) {
  ...
  var bgSize = content ? calculateTooltipSize(props, content, ctx) : 0; // todo: check content null
...
// todo
function getPositionBg(x, bgSize, width) {
  if (x < bgSize.width / 2) {
    x = 0;
  } else if (x > width - bgSize.width / 2) {
    x = width - bgSize.width;
  } else {
    x -= bgSize.width / 2;
  }
  return { x };
